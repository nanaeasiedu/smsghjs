...
function TopUp (apiKey) {
  if (!apiKey) {
    throw new SMSGHError('Provie an api key')
  }

  if (typeof apiKey !== 'string') {
    throw new SMSGHError('Make sure the provided ' + apiKey + ' is a string')
  }
...
TopUp.prototype.topItUp = function (opts, callback) {
  if (!opts || typeof opts !== 'object') {
    callback(new SMSGHError('Make sure the provied options is an object'), null)
  }
  ...
  if (!validNetwork) {
    callback(new SMSGHError('Please make sure the network type is either mtn, vodafone, expresso, airtel, tigo or glo'), null)
  }
